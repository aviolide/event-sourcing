...
// Populate the env vars the wallet service's Zod schema requires.
//
...
// seed the wallet service's expected configuration.
//
...
process.env.DB_NAME = process.env.DB_NAME ?? 'wallet_test';
process.env.DB_USERNAME = process.env.DB_USERNAME ?? 'postgres';
process.env.DB_PASSWORD = process.env.DB_PASSWORD ?? 'postgres';
process.env.KAFKA_BROKER = process.env.KAFKA_BROKER ?? 'localhost:9092';
process.env.KAFKA_CLIENT_ID = process.env.KAFKA_CLIENT_ID ?? 'wallet-test-client';
